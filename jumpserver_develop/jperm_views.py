...
#------------------------------------------------database grant revoke start----------------------------------------------------------------
def AssetList(request):
    if request.method=='POST':
        msg=request.POST.get('dat',None)
        if msg=='G2':
            lt=[]
            print msg
            group_obj = AssetGroup.objects.get(name__contains='G2')
            dataall=group_obj.asset_set.all().values('ip','hostname')
            count=dataall.count()
            for item in dataall:
                key=item.get('ip')
                val=item.get('hostname')
                lt.append((key,val))
            newdat=dict(lt)
            return HttpResponse(json.dumps(newdat))
        elif msg=='G1':
            lt=[]
            print msg
            group_obj = AssetGroup.objects.get(name__contains='G1')
            dataall=group_obj.asset_set.all().values('ip','hostname')
            count=dataall.count()
            for item in dataall:
                key=item.get('ip')
            ...
            lt=[]
            print msg
            group_obj = AssetGroup.objects.get(name__contains='T1')
            dataall=group_obj.asset_set.all().values('ip','hostname')
            count=dataall.count()
            for item in dataall:
                key=item.get('ip')
            ...
            lt=[]
            print msg
            group_obj = AssetGroup.objects.get(name__contains='T3')
            dataall=group_obj.asset_set.all().values('ip','hostname')
            count=dataall.count()
            for item in dataall:
                key=item.get('ip')
            ...
            lt=[]
            print msg
            group_obj = AssetGroup.objects.get(name__contains='T4')
            dataall=group_obj.asset_set.all().values('ip','hostname')
            count=dataall.count()
            for item in dataall:
                key=item.get('ip')
            ...
            lt=[]
            dataall=Asset.objects.all().values('ip','hostname')
            count=dataall.count()
            for item in dataall:
                key=item.get('ip')
                val=item.get('hostname')
...
#            print newdat
        ...
        dataall=Asset.objects.all().values('ip','hostname')
        count=dataall.count()
        return render_to_response('db_perm_test.html', {'data':dataall,'user':'alex','count':count})


def ProcessDatabasePerm(request):
    if request.method=='POST':
        AT=request.POST.get('action')
        PM=request.POST.getlist('perm[]')
        DB=request.POST.get('dbtb')
        USER=request.POST.get('user')
        PWD=request.POST.get('pwd').strip()
        IPLT=request.POST.getlist('iplist[]')
        print AT
        print PM
        print DB
        print USER
        print IPLT
        print PWD
        if all([AT,PM,DB,USER,IPLT]) and DB!="例如:gametool.*" and USER!="例如:'test_user'@'192.168.0.1'":
            if len(PM)>1:
                PMstr= ','.join(PM)
            else:
                PMstr=PM.pop()
            try:
                if AT=='grant':
                    if PWD:
                        sql='%s %s ON %s TO %s IDENTIFIED BY "%s";flush privileges;' % (AT,PMstr,DB,USER,PWD)
                    else:
                        pass
                        return HttpResponse('执行终止,密码不能为空')
                else:
                    sql='%s %s ON %s FROM %s;flush privileges;' % (AT,PMstr,DB,USER)

                print sql
                fail_rt=OrderedDict([('type',AT),('fail',{})])
                print '------------------error 1 start--------------------------'
                for ip in IPLT:
                    hostname = ip
                    port = 62919
                    username = 'jmsqx'
                    key_file = '/data/key/jmsqx_rsa'
                    print hostname,port,username,key_file
                    try:
                        print 'get this location 1'
                    	key = paramiko.RSAKey.from_private_key_file(key_file)
                        print 'get this location 2'
                    	s = paramiko.SSHClient()
                        print 'get this location 3'
                    	s.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                        print 'get this location 4'
                    	s.load_system_host_keys()
                        print 'get this location 5'
                        print 'get this location 6'
                    	s.connect(hostname,port,username,pkey=key,timeout=5)
                        print 'get this location 7'
                    	stdin,stdout,stderr = s.exec_command("ps aux|grep mysqld|grep -v mysqld_safe|awk -Fsocket '{print $2}'|awk -F'--port=' '{print $2}'")
                    	rt_data=stdout.readlines()
                    	pure_data=map(lambda x:x.strip('\n'),rt_data)
                    	Plist=filter(lambda x:bool(x),pure_data)
                    	print Plist 
                    	s.close()
                    except Exception as f:
                        print f
                        fail_rt['fail'][ip]='ssh connect '+str(f)
                        continue
                    for P in Plist:
                        try:
                            conn=MySQLdb.connect(host=ip,user='qx_dbadmin',passwd='q2@t#jY8ph4nu9Ani&rZ35fS%g',port=int(P),db='mysql')
                            cursor=conn.cursor()
                            cursor.execute(sql)
			    cursor.close()
                            conn.commit()
			    ...
			    print c
                            fail_rt['fail'][ip]=c[1][0:30]
                print '------------------error 1 end------------------------'
                if fail_rt['fail']:
                    return HttpResponse(json.dumps(fail_rt))
                else:
                    return HttpResponse('%s,all ok' % AT)
            except Exception as e:
                return HttpResponse(e)
        elif all([AT,USER,IPLT]) and USER!="例如:'test_user'@'192.168.0.1'" and USER!="root@'127.0.0.1'" and USER!="root@'localhost'":
            sql='%s %s' % (AT,USER)
            print sql
            try:
                fail_rt=OrderedDict([('type',AT),('fail',{})])
            ...
            return HttpResponse('条件不满足，执行终止 ，请检查')
    else:
        return render_to_response('assetlist.html')
#------------------------------------------------database grant revoke end----------------------------------------------------------------
